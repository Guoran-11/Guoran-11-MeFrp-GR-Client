...
def get_client():
    ...
    client = MEFrpClient(token=token, user_agent='MeFrp-GR-Client/1.0.0')
    return client
...
def api_login():
    ...
    try:
        ...
        if result.get('code') == 200:
            # 登录成功，token 可能在 Set-Cookie 中，尝试从 cookie 获取
            token = None

            # 方式1：从响应的 Set-Cookie 获取 token
            ...
            # 方式2：如果 cookie 中没有，用 MEFrpClient 登录获取内部 token
            if not token:
                try:
                    client = MEFrpClient(user_agent='MeFrp-GR-Client/1.0.0')
                    ...
                    # 尝试从 client 对象内部获取
                        ...
                        token = client.token
                    elif hasattr(client, '_token') and client._token:
                        token = client._token
                except Exception as inner_err:
                    print(f"[登录] MEFrpClient 备用登录失败: {inner_err}")

            # 方式3：如果还是没有 token，尝试通过 user_info 接口验证并使用原始凭证创建带 token 的 client
            if not token:
                print(f"[登录] 未从响应中提取到 token，尝试备用方案...")
                # 创建一个带登录状态的 client
                client = MEFrpClient(user_agent='MeFrp-GR-Client/1.0.0')
                try:
                    login_res = client.login(username, pwd, captcha_token)
                    if login_res:
                        result = login_res
                        if isinstance(login_res, dict) and "token" in login_res:
                            token = login_res["token"]
                except Exception as e2:
                    print(f"[登录] 备用登录异常: {e2}")

            if token:
                session['token'] = token
                print(f"[登录] 用户 {username} 登录成功，token 已保存")
                return jsonify({
                    'code': 200,
                    'message': '登录成功',
                    'token': token,
                ...
                # 即使没拿到 token，登录也成功了（API 返回 200）
                # 标记为已登录，后续请求通过 MEFrpClient 处理
                session['_login_username'] = username
                session['_login_password'] = pwd  # 用于后续自动重新登录
                print(f"[登录] 用户 {username} 登录成功但未获取到 token")
                return jsonify({
                    'code': 200,
...
def _read_frpc_output(pipe, prefix):
    ...
    try:
        for line in iter(pipe.readline, ''):
            if line:
                frpc_logs.append(f"[{prefix}] {line.rstrip()}")
...
def _clean_toml_config(toml_content):
    """清理 TOML 配置，移除可能会导致 mefrpc 崩溃的 plugin 段"""
    if not toml_content:
        return toml_content
